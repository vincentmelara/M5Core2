...
String payload = "";
bool isRequestInProgress = false; // Flag to track if request is in progress
...
void setup()
{
    ...
    printCenteredText("waiting for payload...");
    loopMainScreen(); 
    updateDisplay(payload);
}

unsigned long lastUpdateTime = 0;  // To store the last update time
const unsigned long updateInterval = 5000;  // Time interval in milliseconds (2 seconds)
...
void printCenteredText(const String& text) {
    int16_t textWidth = M5.Lcd.textWidth(text);  // Get the width of the text
    int16_t textHeight = M5.Lcd.fontHeight();   // Get the height of the text

    // Calculate the center position
    int x = (M5.Lcd.width() - textWidth) / 2;
    int y = (M5.Lcd.height() - textHeight) / 2;

    // Set the text properties and print the text in the center
    M5.Lcd.setTextSize(2);  // Set the text size (can be changed)
    M5.Lcd.setTextColor(WHITE);  // Set the text color (change as needed)
    M5.Lcd.setCursor(x, y);  // Set the cursor to the calculated position
...
void loopMainScreen() {
    ...
    http.addHeader("Connection", "keep-alive");  // Keep the connection alive
...
void makeDataAverageRequest() {
    String url = URL_GCF_AVG + "?userId=" + selectedUserId +
    ...
    if (code == 200) {
        ...
        Serial.println("Response: " + payload);  // Optional debug
        showAverageResults(payload);            // Call display logic
    } else {
        M5.Lcd.fillScreen(BLACK);
        M5.Lcd.setTextColor(RED);
        M5.Lcd.setCursor(10, 10);
...
void showAverageResults(String json) {
    ...
    M5.Lcd.printf("To: %s\n", doc["maxTime"] | "N/A");
    M5.Lcd.printf("Span: %d sec\n", doc["secondsElapsed"] | 0);
    M5.Lcd.printf("Points: %d\n", doc["numPoints"] | 0);
    M5.Lcd.printf("Rate: %.2f/s\n", doc["rate"] | 0.0);
}
